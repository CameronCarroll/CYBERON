...
if __name__ == "__main__":
    # Example usage
    try:
        engine = CyberneticsQueryEngine("cybernetics_ontology_processed.json")
        
        # Print summary
        summary = engine.generate_ontology_summary()
        ...
        # Example query - find information about cybernetics
        cybernetics_info = engine.query_entity("cybernetics")
        print("\nCybernetics Information:")
        print(f"Attributes: {cybernetics_info['attributes']}")
        print(f"Incoming connections: {len(cybernetics_info['incoming'])}")
        print(f"Outgoing connections: {len(cybernetics_info['outgoing'])}")
        
        # Example query - find paths between concepts
        paths = engine.find_paths("information_theory", "transformers")
        print("\nPaths from Information Theory to Transformers:")
        for i, path in enumerate(paths):
            print(f"Path {i+1}: {' -> '.join(node['label'] for node in path)}")
        
        # Example query - find sections by topic
        feedback_sections = engine.find_section_by_topic("feedback")
        print("\nSections mentioning 'feedback':")
        for section in feedback_sections:
            print(f"Section {section['section_num']}: {section['title']}")
            for match in section['subsection_matches']:
                print(f"  - {match['name']}")
